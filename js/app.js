...
async function renderHome() {
  ...
  if (!perfilCache) await loadPerfil();
  const homeHTML = `
  <div class="home-container">
    <div class="home-header">
      <h3>${escapeHtml(perfilCache.nome)}</h3>
      <div class="home-subtitle">${escapeHtml(perfilCache.subtitulo)}</div>
    </div>
    <section class="home-section">
      <h3>${escapeHtml(perfilCache.tituloSecao)}</h3>
      <p style="white-space:pre-line;">${escapeHtml(perfilCache.bio)}</p>
      ${ perfilCache.lattes ? `
        <div class="lattes-link mt-3">
          <i class="fa fa-external-link" aria-hidden="true"></i>
          <a href="${escapeHtml(perfilCache.lattes)}" target="_blank" rel="noopener noreferrer">Lattes</a>
        </div>
      ` : '' }
    </section>
  </div>
